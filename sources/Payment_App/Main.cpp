...
int main(int argc, char ** argv)
{
	...
	try
	{
		const auto& selfItem = configMgr->GetEnclaveList().GetItem(AppNames::sk_payment);
	...
	try
	{
		...
		enclave = std::make_shared<PaymentApp>(
			ENCLAVE_FILENAME, tokenPath, wlKeyArg.getValue(), *serverCon,
			"Payment Pay Info" + selfAddr + ":" + std::to_string(selfPort));
